...
`lib/jobs/runner.ts` — `runJob` / `runJobDetached` wrap fire-and-forget side effects with bounded
retries; on exhaustion the job is written to `failed_jobs` and drained by `/api/cron/retry-jobs`
(every 5 min, abandons after `MAX_CRON_ATTEMPTS = 8`).

- **Not Inngest**, which §1.4 named. The real defect there is missing retries + silent loss, both
  fixable in-process; Inngest means an account, 2 secrets, and replacing 8 working crons. Revisit
  with Vercel Queues if volume ever justifies a real queue.
- `JobKind` is a **closed union** — a job persisted under an unregistered kind can never be retried.
- The retry cron must call `registerAllJobs()`: the registry is module-level and a cold serverless
  invocation starts empty, so it would otherwise abandon everything it drained.
- `CRON_RETRY` backoff is in **minutes**; `DEFAULT_RETRY`'s 5s cap is meaningless between cron runs.
