...
function rainforestToCard(p: RainforestProduct): ServiceCard {
  ...
  return {
    ...
    imageUrl: p.image,
    price: pricePence > 0
      ? { amount: pricePence, currency: 'USD', displayText: priceText }
      : undefined,
    metadata: {
      retailer: 'Amazon',
      rating: p.rating ?? 0,
...
function buildMockProductCards(ctx: SearchContext): ServiceCard[] {
  ...
  const brand = ctx.intent._phaseA?.extracted?.brand?.replace(/^@/, '').toLowerCase() ?? null

  const isElectronics = /laptop|phone|camera|headphone|speaker|tv|tablet/.test(query)
  const isFashion = /shoe|jacket|bag|watch|dress|shirt|sneaker|trainer/.test(query)
  const isHome = /sofa|desk|chair|lamp|mattress|rug/.test(query)

  if (brand && isFashion) return brandedFashionCards(brand, query, isPremium, isBudget)
...
// Verified Unsplash photos per brand — the card must not show a competitor's
// logo (e.g. a Nike swoosh on an "@adidas" search). Unknown brands get the
// logo-free neutral shot. Live imagery requires RAINFOREST_API_KEY.
const BRAND_IMAGES: Record<string, string> = {
  ...
  puma: 'https://images.unsplash.com/photo-1608231387042-66d1773070a5?w=400',
  'new balance': 'https://images.unsplash.com/photo-1539185441755-769473a23570?w=400',
  newbalance: 'https://images.unsplash.com/photo-1539185441755-769473a23570?w=400',
}
const NEUTRAL_SHOE_IMAGE = 'https://images.unsplash.com/photo-1560769629-975ec94e6a86?w=400'

// Brand-scoped mock — every card carries the requested brand so brand searches
// (e.g. "@nike running shoes") stay on-brand without a live product API.
function brandedFashionCards(brand: string, query: string, premium: boolean, budget: boolean): ServiceCard[] {
  const brandName = brand.charAt(0).toUpperCase() + brand.slice(1)
  const isRunning = /run|jog|marathon|trail/.test(query)
  const isShoes = isRunning || /shoe|sneaker|trainer/.test(query)
  const searchTerm = encodeURIComponent(`${brand} ${isRunning ? 'running shoes' : isShoes ? 'trainers' : 'apparel'}`)
  const deepLinkUrl = `https://www.google.com/search?tbm=shop&q=${searchTerm}`

  const models = isRunning
    ? [
        { name: 'Road Racer Elite', desc: 'Responsive foam · Breathable mesh · Race-day light', base: 15900 },
        { name: 'Daily Trainer Max', desc: 'Cushioned ride · Durable outsole · Everyday miles', base: 11900 },
        { name: 'Trail Runner GTX', desc: 'Waterproof · Aggressive grip · All-terrain stability', base: 13900 },
      ]
    : isShoes
      ? [
          { name: 'Court Classic', desc: 'Leather upper · Heritage silhouette · All-day comfort', base: 9500 },
          { name: 'Street Low', desc: 'Suede overlays · Cushioned midsole · Versatile colourways', base: 8500 },
        ]
      : [
          { name: 'Performance Tee', desc: 'Sweat-wicking · Lightweight knit · Athletic fit', base: 3200 },
          { name: 'Training Shorts', desc: 'Four-way stretch · Zip pocket · Quick-dry', base: 3800 },
        ]

  return models.map((m, i) => {
    const amount = premium ? Math.round(m.base * 1.4) : budget ? Math.round(m.base * 0.6) : m.base
    return {
      id: `prod-brand-${brand}-${i + 1}`,
      serviceType: 'products' as const,
      vendorId: `prod-brand-${brand}-${i + 1}`,
      vendorType: 'shopping',
      displayName: `${brandName} ${m.name}`,
      description: m.desc,
      imageUrl: BRAND_IMAGES[brand] ?? NEUTRAL_SHOE_IMAGE,
      price: { amount, currency: 'USD', displayText: `$${(amount / 100).toFixed(amount % 100 === 0 ? 0 : 2)}` },
      metadata: { retailer: brandName, rating: 4.6 + i * 0.1, reviewCount: 5200 - i * 900, inStock: true, deliveryDays: 2, brand: brandName, category: isShoes ? 'trainers' : 'apparel' },
      bookingPayload: { sku: `prod-brand-${brand}-${i + 1}` },
      isBookable: false,
      deepLinkUrl,
      ctaLabel: `Shop ${brandName}`,
    }
  })
}
  ...
  return [
    {
      ...
      price: premium ? { amount: 34999, currency: 'USD', displayText: '$349.99' } : budget ? { amount: 5999, currency: 'USD', displayText: '$59.99' } : { amount: 12999, currency: 'USD', displayText: '$129.99' },
    ...
    },
    {
      ...
      price: premium ? { amount: 199900, currency: 'USD', displayText: '$1,999' } : budget ? { amount: 44999, currency: 'USD', displayText: '$449.99' } : { amount: 114900, currency: 'USD', displayText: '$1,149' },
    ...
    },
    {
      ...
      price: premium ? { amount: 259900, currency: 'USD', displayText: '$2,599' } : budget ? { amount: 9999, currency: 'USD', displayText: '$99.99' } : { amount: 74900, currency: 'USD', displayText: '$749' },
  ...
  return [
    {
      ...
      price: premium ? { amount: 24900, currency: 'USD', displayText: '$249' } : budget ? { amount: 3500, currency: 'USD', displayText: '$35' } : { amount: 8500, currency: 'USD', displayText: '$85' },
    ...
    },
    {
      ...
      price: premium ? { amount: 14000, currency: 'USD', displayText: '$140' } : budget ? { amount: 2500, currency: 'USD', displayText: '$25' } : { amount: 9000, currency: 'USD', displayText: '$90' },
  ...
  return [
    {
      ...
      price: premium ? { amount: 149900, currency: 'USD', displayText: '$1,499' } : budget ? { amount: 22999, currency: 'USD', displayText: '$229.99' } : { amount: 44900, currency: 'USD', displayText: '$449' },
  ...
  return [
    {
      ...
      price: premium ? { amount: 69900, currency: 'USD', displayText: '$699' } : budget ? { amount: 19999, currency: 'USD', displayText: '$199.99' } : { amount: 44900, currency: 'USD', displayText: '$449' },
    ...
    },
    {
      ...
      price: premium ? { amount: 37999, currency: 'USD', displayText: '$379.99' } : budget ? { amount: 9999, currency: 'USD', displayText: '$99.99' } : { amount: 15999, currency: 'USD', displayText: '$159.99' },
